<script lang="ts">
    ...
    function selected(id: string) {
        current_connection = connections_store.connections.find((obj) => obj.id === id) || new_connection('', Protocols.MQTT, '', 1883);
    }
...
<div>
	<h2>MQTT Connection <div>{current_connection.protocol}{current_connection.host}:{current_connection.port}</div></h2>
    <h2>Connections:</h2>
    <button onclick={addConnection}>Add</button>
    <div>
    {#each connections_store.connections as connection}
        <ConnectionItem connection={connection} onselect={selected} />
    {/each}
    </div>
{#if advanced}
    <table>
        ...
        {#each subscriptions_store.subscriptions as subscription}
            <SubscriptionItem subscription={subscription} ondelete={delete_subscription} />
        {/each}
        </tbody>
    </table>
