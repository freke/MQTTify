<script lang="ts">
	...
	import { ConnecitonStatus, ConnectionHealth, connection_status } from '$lib/stores/connection.svelte';
	...
	function connected() {
		...
		let subscriptions =  (current_connection.connection_options?.subscripptions || []).map((s) => {return {topic: s.topic, qos: s.qos}});
		console.log(subscriptions);
		invoke('subscribe',
			{subscriptions: subscriptions}
		);

		if(current_connection.connection_options && current_connection.connection_options.keepalive > 0) {
...
{/if}
<h2>Topics</h2>
<TopicsTree topic={topic_root} indent={0} onselecttopic={(topic: Topic) => { selected_topic = topic } }/>
{/if}
